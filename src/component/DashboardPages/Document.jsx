...
function DocumentLibrary() {
  ...
  const [documents, setDocuments] = useState([
    ...
    },
    {
      ...
      owner: "David Wilson",
      version: "2.1",
      linkedTo: ["Development Phase", "Infrastructure Setup"],
      approvers: [
        { name: "Jane Cooper", status: "Approved", date: "Mar 18, 2024" },
        { name: "Robert Fox", status: "Approved", date: "Mar 19, 2024" },
      ],
      versions: [
        { version: "2.1", date: "Mar 20, 2024", author: "David Wilson", notes: "Final architecture" },
        { version: "2.0", date: "Mar 15, 2024", author: "David Wilson", notes: "Updated based on technical review" },
        { version: "1.0", date: "Mar 10, 2024", author: "David Wilson", notes: "Initial architecture" },
      ],
    },
  ]);
  ...
  const getDocumentIcon = (type) => {
    ...
    return (
      <svg className={`h-6 w-6 ${colors[type] || "text-gray-500"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const getApprovalStatusIcon = (status) => {
    const icons = {
      Approved: { class: "text-green-500", path: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" },
      Pending: { class: "text-yellow-500", path: "M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" },
    };
    const { class: iconClass, path } = icons[status] || { class: "text-gray-500", path: "M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" };
    return (
    ...
    <div className="container mx-auto py-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center">
          <div className="bg-[#00308F]/10 p-2 rounded-lg mr-3">
            <svg className="h-6 w-6 text-[#00308F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
          ...
          {["all", "stages", "milestones", "training"].map((tab) => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium ${
                activeTab === tab ? "border-b-2 border-[#00308F] text-[#00308F]" : "text-gray-500 hover:text-gray-700"
              }`}
            ...
            {documents.map((doc) => (
              <div
                ...
                className={`p-4 border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors ${
                  selectedDocument?.id === doc.id ? "border-[#00308F] bg-[#00308F]/10" : "border-gray-200"
                }`}
        ...
        {activeTab === "stages" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {stages.map((stage) => (
              <div key={stage.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow">
              ...
              <div key={milestone.id} className="p-4 border rounded-lg">
                ...
                <div className="mt-3 flex justify-between items-center">
                  <div className="text-sm text-gray-500">
                    {documents.filter((d) => d.linkedTo.includes(milestone.name)).length} linked documents
                  </div>
                  <button
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {trainingMaterials.map((material) => (
                  <tr key={material.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#00308F]">{material.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{material.audience}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{material.format}</td>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-3 rounded-md space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm font-medium">{selectedDocument.owner}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm">{getStatusBadge(selectedDocument.status)}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div>
                  ...
                  {showLinkingPanel ? (
                    <div className="bg-[#00308F]/10 p-3 rounded-md space-y-3">
                      <div>
                        <label className="text-xs font-medium text-[#00308F]">Link to Milestone</label>
                        <select
                          value={linkForm.milestone}
                          onChange={(e) => setLinkForm({ ...linkForm, milestone: e.target.value })}
                      ...
                      <div>
                        <label className="text-xs font-medium text-[#00308F]">Link to Stage</label>
                        <select
                      ...
                      <div>
                        <label className="text-xs font-medium text-[#00308F]">Link to Task</label>
                        <select
                          value={linkForm.task}
                          onChange={(e) => setLinkForm({ ...linkForm, task: e.target.value })}
                          className="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00308F]"
                        >
                    ...
                    <div className="bg-gray-50 p-3 rounded-md">
                      {selectedDocument.linkedTo.length > 0 ? (
                        <div className="space-y-2">
                          {selectedDocument.linkedTo.map((link, index) => (
                            <div key={index} className="flex justify-between items-center">
                              <div className="flex items-center">
                                <svg className="h-3 w-3 text-[#00308F] mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    strokeWidth="2"
                        ...
                        <div className="text-sm text-gray-500 text-center py-2">No links yet</div>
                      )}
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {showApprovalFlow ? (
                    <div className="bg-[#00308F]/10 p-3 rounded-md space-y-3">
                      <div>
                        <label className="text-xs font-medium text-[#00308F]">Add Approver</label>
                        <select
                      ...
                      <div>
                        <label className="text-xs font-medium text-[#00308F]">Approval Level</label>
                        <select
                          value={approvalForm.level}
                          onChange={(e) => setApprovalForm({ ...approvalForm, level: e.target.value })}
                          className="mt-1 block w-full border border-gray-300 rounded-md p-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00308F]"
                        >
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          onChange={(e) => setApprovalForm({ ...approvalForm, notify: e.target.checked })}
                          className="h-4 w-4 text-[#00308F] border-gray-300 rounded focus:ring-[#00308F]"
                        />
                        <label htmlFor="notify" className="ml-2 text-xs text-[#00308F]">
                          ...
                          {selectedDocument.approvers.map((approver, index) => (
                            <div key={index} className="flex justify-between items-center">
                              ...
                              <div className="flex items-center">
                                ...
                                <span className="text-xs ml-1">{approver.status}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <div className="text-sm text-gray-500 text-center py-2">No approvers yet</div>
                      )}
                    </div>
                  )}
                </div>
                    ...
                    <div className="bg-gray-50 p-3 rounded-md space-y-3 max-h-60 overflow-y-auto">
                      {selectedDocument.versions.map((version, index) => (
                        <div key={index} className="border-b border-gray-200 pb-2 last:border-0 last:pb-0">
                          ...
                          <div className="mt-1 text-xs text-gray-500">By {version.author}</div>
                    ...
                    <div className="bg-gray-50 p-3 rounded-md">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center">
                          ...
                          <span className="text-sm font-medium">Current: v{selectedDocument.version}</span>
                        </div>
                        <span className="text-xs text-gray-500">{selectedDocument.modified}</span>
                      </div>
                      <div className="mt-1 text-xs text-gray-500">{selectedDocument.versions.length} previous versions</div>
                    </div>
                  )}
                </div>
              </div>
